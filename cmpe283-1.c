...
#define DRIVER_AUTHOR "Het Jagani <hetpatel572@gmail.com>"
#define DRIVER_DESC   "Module to print vmx capabilities"
...
void detect_vmx_features(void)
{
	...
	if(!(hi & (1<<31))) {
		printk("======Secondary Proc Based Controls not available.======");
	} else {
		printk("======Secondary Proc Based Controls available.======");
...
static int __init cmpe283_module_init(void) {
	printk(KERN_INFO "CMPE 283 Assignment 1 Module Start\n");
	detect_vmx_features();
	return 0;
}

static void __exit cmpe283_module_exit(void) {
	printk(KERN_INFO "CMPE 283 Assignment 1 Module Exits\n");
}

module_init(cmpe283_module_init);
module_exit(cmpe283_module_exit);
